...
const ids=new Set();         // for adding item to cart only once;
...
const addToCart=(id)=>{
  if(ids.has(id)){
    ...
    console.log(products[id-1].title,products[id-1].price);
  }
  ids.add(id);
...
<div className="card h-200" >
 ...
 <h2><span class="badge text-bg-danger rounded-circle"  >{value.discount}{value.offer}
  ...
  <button type="button" class="btn btn-outline-warning fw-bold"  onClick={() => addToCart(value.id)}>Add To Cart</button>
</div>
  </Col>
    );
  })
